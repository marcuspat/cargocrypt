...
use super::{GitCryptConfig, GitError, GitRepo, GitResult};
...
impl GitAttributes {
    ...
    pub async fn load(&mut self) -> GitResult<()> {
        if self.attributes_path.exists() {
            let content = fs::read_to_string(&self.attributes_path)
                .await
                .map_err(|e| {
                    GitError::AttributesFailed(format!("Failed to read .gitattributes: {}", e))
                })?;
    ...
    pub async fn save(&self) -> GitResult<()> {
        ...
        fs::write(&self.attributes_path, content)
            .await
            .map_err(|e| {
                GitError::AttributesFailed(format!("Failed to write .gitattributes: {}", e))
            })?;
    ...
    pub async fn add_cargocrypt_patterns(&mut self) -> GitResult<()> {
        let patterns: Vec<(String, String)> = self.config.patterns.clone().into_iter().collect();
    ...
    pub async fn configure_filters(&self, _git_config: &GitCryptConfig) -> GitResult<()> {
        ...
        let mut config_content = if git_config_path.exists() {
            fs::read_to_string(&git_config_path).await.map_err(|e| {
                GitError::AttributesFailed(format!("Failed to read git config: {}", e))
            })?
        } else {
            String::new()
        };
        ...
        for (filter_name, filter_config) in &self.config.filters {
            let filter_section = format!(
                "\n[filter \"{}\"]\n\tclean = {}\n\tsmudge = {}\n\trequired = {}\n",
                filter_name, filter_config.clean, filter_config.smudge, filter_config.required
            );
        ...
        fs::write(&git_config_path, config_content)
            .await
            .map_err(|e| {
                GitError::AttributesFailed(format!("Failed to write git config: {}", e))
            })?;
    ...
    pub async fn remove_cargocrypt_patterns(&mut self) -> GitResult<()> {
        self.patterns
            .retain(|p| !p.attribute.contains("cargocrypt"));
        Ok(())
    }
    ...
    pub async fn update_smart_patterns(&mut self) -> GitResult<()> {
        ...
        for env_file in ["*.env", "*.env.local", "*.env.production", "*.env.staging"] {
            if self.has_files_matching_pattern(env_file).await? {
                discovered_patterns.push((
                    env_file.to_string(),
                    self.config.default_encrypt_attr.clone(),
                ));
            }
        }
    ...
    async fn has_files_matching_pattern(&self, pattern: &str) -> GitResult<bool> {
        ...
        for entry in WalkDir::new(workdir).max_depth(3) {
            let entry =
                entry.map_err(|e| GitError::AttributesFailed(format!("Walk error: {}", e)))?;
            let path = entry.path();
    ...
    pub fn validate_attributes(&self) -> GitResult<Vec<String>> {
        ...
        for (i, pattern1) in self.patterns.iter().enumerate() {
            for (j, pattern2) in self.patterns.iter().enumerate() {
                if i != j
                    ...
                    && pattern1.attribute != pattern2.attribute
                {
        ...
        for pattern in &self.patterns {
            if pattern.pattern == "*" {
                warnings.push(
                    "Pattern '*' will encrypt all files - this may not be intended".to_string(),
                );
            }
        }
...
mod tests {
    ...
    async fn test_pattern_operations() {
        ...
        attributes
            .add_pattern("*.secret", "cargocrypt-encrypt")
            .await
            .unwrap();
    ...
    async fn test_save_and_load() {
        ...
        attributes
            .add_pattern("*.test", "cargocrypt-encrypt")
            .await
            .unwrap();
