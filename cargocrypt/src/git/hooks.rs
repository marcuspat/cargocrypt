        ...
        for hook_type in &cargocrypt_hooks {
            ...
            if hook_path.exists() {
                ...
                if content.contains("CargoCrypt") {
                    fs::remove_file(&hook_path).await.map_err(|e| {
                        GitError::HookFailed(format!("Failed to remove hook: {}", e))
                    })?;
...
impl SecretDetectionHook {
    ...
    pub async fn detect_secrets_in_staged_files(
        &self,
                ...
                match fs::read_to_string(file_path).await {
                    Ok(content) => {
                        ...
                        let content_validation =
                            validator.validate_file_content(content.as_bytes(), file_path);
                        for warning in &content_validation.warnings {
                            warn!("File content warning for {}: {}", file_path, warning);
                        }

                        match self
                            .detect_secrets_in_content(&content, file_path, config)
                            .await
                        {
                            Ok(file_detections) => {
        ...
        info!(
            "Secret detection completed: {} detections found",
            detections.len()
        );
        Ok(detections)
    }
    ...
    fn is_binary_file(&self, file_path: &str) -> bool {
        let binary_extensions = [
            ".exe", ".dll", ".so", ".dylib", ".a", ".o", ".obj", ".jpg", ".jpeg", ".png", ".gif",
            ".bmp", ".tiff", ".mp3", ".mp4", ".avi", ".mov", ".wmv", ".zip", ".tar", ".gz", ".rar",
            ".7z", ".pdf", ".doc", ".docx", ".xls", ".xlsx", ".ppt", ".pptx",
        ];
...
mod tests {
    ...
    async fn test_hook_installation() {
        ...
        hooks
            .install_hook(HookType::PreCommit, Box::new(hook))
            .await
            .unwrap();
