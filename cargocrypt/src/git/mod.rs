...
pub use attributes::{AttributeConfig, EncryptionPattern, GitAttributes};
pub use config::{GitCryptConfig, IntegrationMode, RepositorySetup};
pub use hooks::{GitHooks, HookConfig, HookType, SecretDetectionHook};
pub use ignore::{GitIgnoreManager, IgnoreConfig, IgnorePattern};
pub use repo::{GitRepo, GitRepoError, GitRepoResult};
pub use storage::{EncryptedStorage, GitObjectStorage, StorageRef};
pub use team::{KeyShareConfig, TeamKeySharing, TeamMember};
...
impl GitIntegration {
    ...
    async fn setup_gitignore(&self) -> GitResult<()> {
        ...
        ignore_manager.add_pattern(".cargocrypt/").await?;
        ignore_manager
            .add_pattern("!.cargocrypt/config.toml")
    ...
    async fn setup_attributes(&self) -> GitResult<()> {
        ...
        attributes
            ...
            .await?;
        attributes
            .add_pattern("*.key", "cargocrypt-encrypt")
            .await?;
        attributes
            .add_pattern("secrets/*", "cargocrypt-encrypt")
            .await?;
        attributes
            .add_pattern("config/secrets.*", "cargocrypt-encrypt")
            .await?;
    ...
    async fn setup_hooks(&self) -> GitResult<()> {
        ...
        hooks
            .install_hook(HookType::PreCommit, Box::new(detection_hook))
            .await?;
    ...
    pub async fn encrypt_and_stage<P: AsRef<Path>>(
        &self,
        path: P,
        password: &str,
    ) -> GitResult<PathBuf> {
    ...
    pub async fn decrypt_from_git<P: AsRef<Path>>(
        &self,
        encrypted_path: P,
        password: &str,
    ) -> GitResult<PathBuf> {
