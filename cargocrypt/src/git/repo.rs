...
impl GitRepo {
        ...
        match Repository::discover(path) {
            Ok(repo) => {
                let workdir = repo
                    .workdir()
                    .ok_or_else(|| {
                        GitRepoError::InvalidState("Bare repository not supported".to_string())
                    })?
                    .to_path_buf();
                Ok(Self { repo, workdir })
            }
    ...
    pub fn has_cargocrypt_config(&self) -> bool {
        self.workdir.join(".cargocrypt").exists() || self.workdir.join("Cargo.toml").exists()
    ...
    fn make_relative_path<P: AsRef<Path>>(&self, path: P) -> GitRepoResult<PathBuf> {
        ...
        if path.is_absolute() {
            path.strip_prefix(&self.workdir)
                .map(|p| p.to_path_buf())
                .map_err(|_| {
                    GitRepoError::InvalidState(format!(
