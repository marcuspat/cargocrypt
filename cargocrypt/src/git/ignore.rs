...
impl GitIgnoreManager {
    ...
    pub async fn load(&mut self) -> GitResult<()> {
        if self.gitignore_path.exists() {
            let content = fs::read_to_string(&self.gitignore_path)
                .await
                .map_err(|e| {
                    GitError::StorageFailed(format!("Failed to read .gitignore: {}", e))
                })?;

            self.patterns = content.lines().map(IgnorePattern::from_line).collect();
        }
    ...
    pub async fn save(&self) -> GitResult<()> {
        ...
        if self.config.backup_existing && self.gitignore_path.exists() {
            ...
            fs::copy(&self.gitignore_path, backup_path)
                .await
                .map_err(|e| {
                    GitError::StorageFailed(format!("Failed to backup .gitignore: {}", e))
                })?;
        }
    ...
    pub async fn add_pattern(&mut self, pattern: &str) -> GitResult<()> {
        if !self.has_pattern(pattern) {
            self.patterns
                .push(IgnorePattern::Ignore(pattern.to_string()));
        }
        Ok(())
    }
    ...
    pub async fn add_comment(&mut self, comment: &str) -> GitResult<()> {
        self.patterns
            .push(IgnorePattern::Comment(format!("# {}", comment)));
        Ok(())
    }
    ...
    pub async fn add_cargocrypt_patterns(&mut self) -> GitResult<()> {
        ...
        if !self.has_cargocrypt_patterns() {
            ...
            self.patterns
                .push(IgnorePattern::Comment(self.config.section_header.clone()));
        }
    ...
    pub async fn update_smart_patterns(&mut self) -> GitResult<()> {
        ...
        for dir in &secret_dirs {
            ...
            if dir_path.exists() && dir_path.is_dir() {
                ...
                self.add_comment(&format!("Auto-detected secret directory: {}", dir))
                    .await?;
            }
        }
            ...
            for pattern in &ignore_patterns {
                if self.pattern_matches_file(pattern, file) {
                    warnings.push(format!(
                        "Pattern '{}' may ignore important file '{}'",
                        pattern, file
                    ));
                }
            }
...
mod tests {
    ...
    fn test_ignore_pattern_parsing() {
        assert_eq!(
            IgnorePattern::from_line("*.enc"),
            IgnorePattern::Ignore("*.enc".to_string())
        );
        assert_eq!(
            IgnorePattern::from_line("!config.toml"),
            IgnorePattern::Include("config.toml".to_string())
        );
        assert_eq!(
            IgnorePattern::from_line("# Comment"),
            IgnorePattern::Comment("# Comment".to_string())
        );
    ...
    fn test_pattern_cleanup() {
        ...
        manager
            .patterns
            .push(IgnorePattern::Ignore("*.enc".to_string()));
