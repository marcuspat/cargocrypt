...
impl KeyDerivationValidator {
    ...
    pub fn validate_params(
        ...
        if memory_cost < 47104 {
            // ~46MB minimum
            return Err(
                "Memory cost too low - minimum 47104 KiB (46MB) required for security".to_string(),
            );
        }

        if memory_cost > 2097152 {
            // 2GB maximum for practical use
            return Err(
                "Memory cost too high - maximum 2097152 KiB (2GB) for performance".to_string(),
            );
        }
