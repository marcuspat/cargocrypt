...
use cargocrypt::crypto::{
    defaults, CryptoEngine, DerivedKey, EncryptionOptions, PerformanceProfile, PlaintextSecret,
    SecretMetadata, SecretType,
};
...
fn bench_key_derivation(c: &mut Criterion) {
    ...
    for (name, profile) in profiles {
        group.bench_with_input(BenchmarkId::new("argon2", name), &profile, |b, &profile| {
            b.iter(|| {
...
fn bench_encryption_sizes(c: &mut Criterion) {
    ...
    for (name, size) in sizes {
        ...
        let encrypted = engine
            .encrypt_bytes(&data, password, EncryptionOptions::new())
            .unwrap();
    ...
    for &count in &batch_sizes {
        let secrets: Vec<(String, String)> = (0..count)
            .map(|i| {
                (
                    format!("secret_{}", i),
                    format!("secret_data_{}_with_some_content", i),
                )
            })
            .collect();
...
fn bench_password_operations(c: &mut Criterion) {
    ...
    group.bench_function("change_password", |b| {
        b.iter(|| {
            black_box(
                engine
                    .change_password(&encrypted, password, new_password)
                    .unwrap(),
            );
        })
    });
...
fn bench_memory_operations(c: &mut Criterion) {
    ...
    group.bench_function("large_data_encrypt", |b| {
        b.iter(|| {
            ...
            black_box(
                engine
                    .encrypt_bytes(&large_data, password, options)
                    .unwrap(),
            );
        })
    });

    let encrypted_large = engine
        .encrypt_bytes(&large_data, password, EncryptionOptions::new())
        .unwrap();
...
fn bench_concurrent_operations(c: &mut Criterion) {
    ...
    let encrypted = engine
        .encrypt_string(data, password, EncryptionOptions::new())
        .unwrap();
        ...
        b.iter(|| {
            ...
            let handles: Vec<_> = (0..4)
                .map(|_| {
                    ...
                    let password = password.to_string();
                    thread::spawn(move || engine.decrypt_to_string(&encrypted, &password).unwrap())
                })
                .collect();
...
fn bench_performance_targets(c: &mut Criterion) {
    ...
    let encrypted = engine
        .encrypt_string(test_data, password, EncryptionOptions::new())
        .unwrap();
